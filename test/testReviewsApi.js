...
// #2 add a review
  it("should add a review",function(done){
    supertest(server)
    .post('/api/reviews')
    .send({title:"Test Review",link:"https://github.com/ellingworthl/EWD_Assign2_WebAPI",username:"TiggerLady"})
    .expect("Content-type",/json/)
    .expect(201)
    .end(function(err,res){
      res.status.should.equal(201);
      res.body.review.should.have.property('_id');
      res.body.review.username.should.equal('TiggerLady');
      done();
    });
  });
...
// #4 add & delete Review 999
  it("should add and delete Review 999",function(done){
    ...
    .end(function(err,res){
        ...
        superserver
        .put("/api/reviews/"+id)
        .send({title:"Review 999",link:"Review",username:"TiggerLady"})
        .expect("Content-type",/json/)
        .expect(200) // This is HTTP response
        .end(function(err,res){
            superserver
